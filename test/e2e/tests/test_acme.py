...
class TestAcmeDomainValidation:
    def test_create_delete(self, acme_domain_validation, acm_client):
        ...
        # Poll for the resource to become synced (settled VALID/INVALID per
        # the synced.when condition) to avoid timing inconsistencies.
        assert k8s.wait_on_condition(ref, "ACK.ResourceSynced", "True", wait_periods=3)
        ...
        # After the synced wait the validation has settled; example.com
        # cannot be validated so it lands INVALID.
        status = cr["status"].get("status")
        assert status in ("INVALID", "VALID"), \
            f"Expected settled status, got {status}"
